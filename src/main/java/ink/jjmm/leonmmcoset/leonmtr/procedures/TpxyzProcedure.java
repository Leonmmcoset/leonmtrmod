...
public class TpxyzProcedure {
	public static void execute(LevelAccessor world, CommandContext<CommandSourceStack> arguments, Entity entity) {
		...
		if ((entity.level().dimension()) == ResourceKey.create(Registries.DIMENSION, new ResourceLocation("leonmtr:joinworld"))) {
			LeonmtrModVariables.MapVariables.get(world).tpx = new Object() {
				public double getX() {
					try {
			...
			LeonmtrModVariables.MapVariables.get(world).tpy = new Object() {
				public double getY() {
					try {
			...
			LeonmtrModVariables.MapVariables.get(world).syncData(world);
			LeonmtrModVariables.MapVariables.get(world).tpz = new Object() {
				public double getZ() {
					try {
			...
			if (entity instanceof Player _player && !_player.level().isClientSide())
				_player.displayClientMessage(Component.literal(
						("\u8BBE\u7F6E\u6210\u529F\uFF01\u5F53\u524Dxyz\u4E3A" + LeonmtrModVariables.MapVariables.get(world).tpx + "," + LeonmtrModVariables.MapVariables.get(world).tpy + "," + LeonmtrModVariables.MapVariables.get(world).tpz)),
						false);
		} else {
			if (entity instanceof Player _player && !_player.level().isClientSide())
