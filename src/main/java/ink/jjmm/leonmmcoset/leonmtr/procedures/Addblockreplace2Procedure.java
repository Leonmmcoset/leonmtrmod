...
public class Addblockreplace2Procedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if (entity instanceof Player _player) {
			ItemStack _stktoremove = new ItemStack(LeonmtrModItems.WORLDEDITADDBLOCK_3.get());
			_player.getInventory().clearOrCountMatchingItems(p -> _stktoremove.getItem() == p.getItem(), 1, _player.inventoryMenu.getCraftSlots());
		}
		if ((entity.getCapability(LeonmtrModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new LeonmtrModVariables.PlayerVariables())).worldeditcommandr2 == 1) {
			if (("" + world.getBlockState(BlockPos.containing(x, y, z))).contains("[")) {
				{
					...
					entity.getCapability(LeonmtrModVariables.PLAYER_VARIABLES_CAPABILITY, null).ifPresent(capability -> {
						capability.nbtstringr2 = _setval;
		...
		if (entity instanceof ServerPlayer _ent) {
			...
			NetworkHooks.openScreen((ServerPlayer) _ent, new MenuProvider() {
				...
				public Component getDisplayName() {
					return Component.literal("Worldeditcommandreplace");
				}

				@Override
				public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
					return new WorldeditcommandreplaceMenu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(_bpos));
				}
			}, _bpos);
		}
		if (entity instanceof Player _player && _player.containerMenu instanceof Supplier _current && _current.get() instanceof Map _slots) {
			ItemStack _setstack = new ItemStack(ForgeRegistries.ITEMS
					.getValue(new ResourceLocation((((entity.getCapability(LeonmtrModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new LeonmtrModVariables.PlayerVariables())).blockstring)).toLowerCase(java.util.Locale.ENGLISH)))).copy();
